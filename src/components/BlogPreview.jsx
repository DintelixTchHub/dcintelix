    ...
    <section className="py-12 md:py-16 lg:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <span className="inline-block text-[#0D6D63] font-medium text-xs uppercase tracking-wider mb-2">Insights</span>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {samplePosts.map((post) => (
            <motion.article whileHover={{ translateY: -6 }} key={post.id} className="card-modern p-5">
              <div className="text-xs text-[#0D6D63] font-medium uppercase tracking-wider mb-2">{post.category}</div>
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <Link to="/blogs" className="text-[#0D6D63] font-medium">Read</Link>
              </div>
            </motion.article>
          ))}
        </div>
